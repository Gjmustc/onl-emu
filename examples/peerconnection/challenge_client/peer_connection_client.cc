...
int PeerConnectionClient::StartListen(const std::string& ip, int port) {
  ...
  if (err < 0) {
    ...
    RTC_LOG(LS_ERROR) << "Failed to bind listening socket to port " << port;
    return -1;
  }
...
int PeerConnectionClient::StartConnect(const std::string& ip, int port) {
...
void PeerConnectionClient::OnGetMessage(rtc::AsyncSocket* socket) {
  ...
  RTC_LOG(LS_INFO) << "OnGetMessage received " << msg;
  if (msg.compare("BYE[EOF]") == 0) {
    RTC_LOG(LS_INFO) << "OnGetMessage Received hangup message BYE. Disconnecting...";
