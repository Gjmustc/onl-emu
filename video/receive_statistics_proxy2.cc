...
void ReceiveStatisticsProxy::OnRenderedFrame(
  ...
  if (samples >= kMinRequiredSamples) {
    int rendered_fps = round(render_fps_tracker_.ComputeTotalRate());
    RTC_LOG(LS_VERBOSE) << "QoE RenderFramesPerSecond " << rendered_fps << \
       " at time " << clock_->CurrentTime().ms() << " ms";
  }
  ...
  RTC_LOG(LS_VERBOSE) << "QoE ReceivedWidthInPixels " << frame_meta.width << \
       " ReceivedHeightInPixels " << frame_meta.height << \
       " at time " << clock_->CurrentTime().ms() << " ms";
  ...
  if (frame_meta.ntp_time_ms > 0) {
    ...
    if (delay_ms >= 0) {
      RTC_LOG(LS_VERBOSE) << "QoE EndToEndDelayInMs " << delay_ms << \
      " at time " << clock_->CurrentTime().ms() << " ms";
  ...
  // Don't report these 3 metrics unsliced, as more precise variants
  // are reported separately in this method.
  float flow_duration_sec = content_specific_stats->flow_duration_ms / 1000.0;
  if (flow_duration_sec >= metrics::kMinRunTimeInSeconds) {
    int media_bitrate_kbps = static_cast<int>(content_specific_stats->total_media_bytes * 8 /
                                              flow_duration_sec / 1000);
    RTC_LOG(LS_VERBOSE) << "QoE MediaBitrateReceivedInKbps " << media_bitrate_kbps << \
      " at time " << clock_->CurrentTime().ms() << " ms";
  }
