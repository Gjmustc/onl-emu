...
void RemoteEstimatorProxy::IncomingPacket(int64_t arrival_time_ms,
  ...
  RTC_LOG(LS_VERBOSE) << "ONNX IncomingPacket: sequenceNumber " << header.sequenceNumber <<
  ...
  if (time_to_send_bew_message) {
    BweMessage bwe;
    estimation = rl_agent::GetEstimatedBandwidth();
    // estimation = 1000000;
    RTC_LOG(LS_VERBOSE) << "ONNX: BWE " << estimation;
...
bool RemoteEstimatorProxy::TimeToSendBweMessage() {
  ...
  if (time_now - last_bwe_sendback_ms_ > bwe_sendback_interval_ms_) {
