...
void RtpTransportControllerSend::OnBitrateReceivedInBps(
    const float avg_recv_thp) {
  RemoteBitrateReport msg;
  msg.receive_time = Timestamp::Millis(clock_->TimeInMilliseconds());
  msg.bandwidth = DataRate::BitsPerSec(avg_recv_thp);
  task_queue_.PostTask([this, msg]() {
    RTC_DCHECK_RUN_ON(&task_queue_);
    if (controller_) {
      RTC_LOG(LS_VERBOSE)
        << "GCC: RtpTransportControllerSend::OnBitrateReceivedInBps Called, sending " << msg.bandwidth.bps();
      PostUpdates(controller_->OnBitrateReceivedInBps(msg));
    }
  });
}
...
void RtpTransportControllerSend::OnApplicationPacket(const rtcp::App& app) {
  ...
  task_queue_.PostTask([this, bwe]() {
    RTC_DCHECK_RUN_ON(&task_queue_);
    if (controller_) {
      RTC_LOG(LS_VERBOSE) << "ONNX: OnApplicationPacket BWE " << bwe.target_rate;
