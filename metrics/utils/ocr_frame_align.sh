...
align_ocr() {
   ...
   for ((i=${#files[@]}-1; i>=0; i--)); do
      ...
      frame=$(gocr -C 0-9 $cut_folder/_$filename | tr -d '[:space:]' | sed 's/[^0-9]//g')
      # frame=${frame#0}
   ...
   if $CALCULATE_AUDIO_QOE; then
      ffmpeg $FFMPEG_LOG -y -r $FPS -f image2 -i $cut_folder/%04d.jpg -i $wav_ocr $output_ocr
   else
      ffmpeg $FFMPEG_LOG -y -r $FPS -f image2 -i $cut_folder/%04d.jpg -pix_fmt $YUV_PROFILE $output_ocr
   fi
}
